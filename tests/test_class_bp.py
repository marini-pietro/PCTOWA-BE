...
def test_post_class(client, mocker):
    ...
    response = client.post(
        '/class',
        json={"sigla": "4A", "anno": "2024-2025", "email_responsabile": "teacher@example.com"},
...
def test_get_class(client, mocker):
    mocker.patch('api_blueprints.class_bp.fetchall_query', return_value=[{"sigla": "4A"}])
    token = create_access_token(identity={"role": "admin"})
...
def test_get_class_by_email(client, mocker):
    mocker.patch('api_blueprints.class_bp.fetchone_query', return_value={"nome": "Teacher"})
    mocker.patch('api_blueprints.class_bp.fetchall_query', return_value=[{"sigla": "4A", "email_responsabile": "teacher@example.com", "anno": "2024-2025"}])
    token = create_access_token(identity={"role": "admin"})
    response = client.get('/class/teacher@example.com', headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    assert response.json[0]['sigla'] == '4A'
...
def test_fuzzy_search_class(client, mocker):
    mocker.patch('api_blueprints.class_bp.fetchall_query', return_value=[{"sigla": "4A"}])
    token = create_access_token(identity={"role": "admin"})
    response = client.get('/class/fsearch?fnome=4', headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    assert response.json[0]['sigla'] == '4A'

def test_list_classes(client, mocker):
    mocker.patch('api_blueprints.class_bp.fetchall_query', return_value=[{"sigla": "4A"}, {"sigla": "4B"}])
    token = create_access_token(identity={"role": "admin"})
    response = client.get('/class/list', headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
