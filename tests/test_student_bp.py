...
def test_get_student(client, mocker):
    mocker.patch(
        "api_blueprints.student_bp.fetchall_query",
        return_value=[{"matricola": "12345"}],
    )
    token = create_access_token(identity={"role": "admin"})
    response = client.get(
        "/student/class/1", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_delete_student(client, mocker):
    ...
    mocker.patch(
        "api_blueprints.student_bp.fetchone_query", return_value={"matricola": "12345"}
    )
    token = create_access_token(identity={"role": "admin"})
    response = client.delete(
        "/student/12345", headers={"Authorization": f"Bearer {token}"}
    )
