...
def test_get_legalforms(client, mocker):
    mocker.patch('api_blueprints.legalform_bp.fetchall_query', return_value=[{"forma_giuridica": "SRL"}])
...
def test_delete_legalform(client, mocker):
    ...
    mocker.patch('api_blueprints.legalform_bp.fetchone_query', return_value={"forma_giuridica": "SRL"})
    token = create_access_token(identity={"role": "admin"})
    response = client.delete('/legalform/SRL', headers={"Authorization": f"Bearer {token}"})
