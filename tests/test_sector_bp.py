...
def test_get_sectors(client, mocker):
    mocker.patch('api_blueprints.sector_bp.fetchall_query', return_value=[{"settore": "IT"}])
...
def test_delete_sector(client, mocker):
    ...
    mocker.patch('api_blueprints.sector_bp.fetchone_query', return_value={"settore": "IT"})
    token = create_access_token(identity={"role": "admin"})
