...
def test_get_user(client, mocker):
    mocker.patch(
        "api_blueprints.user_bp.fetchall_query",
        return_value=[{"email": "user@example.com"}],
    )
    token = create_access_token(identity={"role": "admin"})
    response = client.get(
        "/user/binded/1", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_delete_user(client, mocker):
    ...
    mocker.patch(
        "api_blueprints.user_bp.fetchone_query",
        return_value={"email": "user@example.com"},
    )
    token = create_access_token(identity={"role": "admin"})
    response = client.delete(
        "/user/user@example.com", headers={"Authorization": f"Bearer {token}"}
    )
